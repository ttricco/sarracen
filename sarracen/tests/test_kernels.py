...
class TestKernels:
    def test_cubicspline(self):
        ...
        assert kernel.weight(0, 1) == 2 / 3
        assert kernel.weight(0, 2) == 10 / (7 * np.pi)
        assert kernel.weight(0, 3) == 1 / np.pi

        # testing kernel values at q = 1
        assert kernel.weight(1, 1) == 1 / 6
        assert kernel.weight(1, 2) == 5 / (14 * np.pi)
        assert kernel.weight(1, 3) == 1 / (4 * np.pi)

        # testing kernel values at q = 2
        assert kernel.weight(2, 2) == 0
        assert kernel.weight(10, 3) == 0
    ...
    def test_quarticspline(self):
        ...
        assert kernel.weight(0, 1) == approx(115 / 192)
        assert kernel.weight(0, 2) == approx(1380 / (1199 * np.pi))
        assert kernel.weight(0, 3) == approx(23 / (32 * np.pi))

        assert kernel.weight(1, 1) == approx(19 / 96)
        assert kernel.weight(1, 2) == approx(456 / (1199 * np.pi))
        assert kernel.weight(1, 3) == approx(19 / (80 * np.pi))

        # these are equivalent to the normalization constants
        assert kernel.weight(1.5, 1) == approx(1 / 24)
        assert kernel.weight(1.5, 2) == approx(96 / (1199 * np.pi))
        assert kernel.weight(1.5, 3) == approx(1 / (20 * np.pi))

        assert kernel.weight(2.5, 2) == 0
        assert kernel.weight(10, 3) == 0
    ...
    def test_quinticspline(self):
        ...
        assert kernel.weight(0, 1) == approx(11 / 20)
        assert kernel.weight(0, 2) == approx(231 / (239 * np.pi))
        assert kernel.weight(0, 3) == approx(11 / (20 * np.pi))

        assert kernel.weight(1, 1) == approx(13 / 60)
        assert kernel.weight(1, 2) == approx(91 / (239 * np.pi))
        assert kernel.weight(1, 3) == approx(13 / (60 * np.pi))

        # these are equivalent to the normalization constants
        assert kernel.weight(2, 1) == approx(1 / 120)
        assert kernel.weight(2, 2) == approx(7 / (478 * np.pi))
        assert kernel.weight(2, 3) == approx(1 / (120 * np.pi))

        assert kernel.weight(3, 2) == 0
        assert kernel.weight(10, 3) == 0
